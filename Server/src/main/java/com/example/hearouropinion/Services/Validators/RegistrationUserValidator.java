...
public class RegistrationUserValidator extends Validator {

    @Override
    public void validate(Map<String, String> toValidate) throws IllegalArgumentException {
        validators.addValidationUnit(new NameValidationUnit());
    ...
    private class NameValidationUnit extends ValidationSingleUnit{

        @Override
        void validate(String parName, String toValidate) {
            if (parName.equals("name")) {
                if (!toValidate.matches("[a-zA-Zа-яА-Я\\s]+")) {
                    throw new IllegalArgumentException(String.format("%s must consist only of latin or cyrillic letters and can include spacebars", parName));
                }
            }
        }
    }
